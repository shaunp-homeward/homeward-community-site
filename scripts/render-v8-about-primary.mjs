...
<section class="ap-questions ap-section" id="our-story-journey"><div class="ap-shell"><div class="ap-two"><div><p class="ap-eyebrow">${esc(data.questions.eyebrow)}</p><h2>${esc(data.questions.heading)}</h2></div><div class="ap-story-card">${data.questions.paragraphs.map(p).join('')}</div></div><div class="ap-timeline">${data.questions.timeline.map((it)=>`<article class="ap-timeline-row"><div class="ap-timeline-label">${esc(it.label)}</div><div><h3>${esc(it.title)}</h3><p>${esc(it.body)}</p></div></article>`).join('')}</div></div></section>

<section class="ap-search ap-section"><div class="ap-shell"><div class="ap-search-head"><p class="ap-eyebrow">${esc(data.search.eyebrow)}</p><h2>${esc(data.search.heading)}</h2>${data.search.paragraphs.map((text)=>`<p class="ap-lead">${esc(text)}</p>`).join('')}</div><div class="ap-carousel" aria-label="Photos from Shaun's spiritual journey">${data.search.photos.map((it)=>`<figure class="ap-photo ${it.contain ? 'contain' : ''}"><div class="ap-photo-frame"><img src="${attr(it.src)}" alt="${attr(it.alt)}"></div><figcaption>${esc(it.caption)}</figcaption></figure>`).join('')}</div></div></section>
...
<section class="ap-missing ap-section"><div class="ap-shell"><div class="ap-missing-card"><p class="ap-eyebrow">${esc(data.missing.eyebrow)}</p><h2>${esc(data.missing.heading)}</h2><p class="ap-missing-body">${esc(data.missing.paragraph1)}</p><p class="ap-missing-body">${esc(data.missing.paragraph2)}</p><p class="ap-missing-signature">${esc(data.missing.signature)}</p></div></div></section>

<section class="ap-trust ap-section" id="trust"><div class="ap-shell"><div class="ap-trust-head"><p class="ap-eyebrow">${esc(data.trust.eyebrow)}</p><h2>${esc(data.trust.heading)}</h2><p class="ap-lead">${esc(data.trust.lead)}</p></div><div class="ap-trust-grid">${data.trust.cards.map((it,i)=>`<article class="ap-trust-card"><div class="ap-trust-num">${i+1}</div><h3>${esc(it.title)}</h3><p>${esc(it.body)}</p></article>`).join('')}</div><div class="ap-trust-actions">${btn(data.trust.linkedin_label,data.trust.linkedin_url,true)}</div>
<div class="ap-founder"><figure><img src="${attr(data.trust.founder_image)}" alt="${attr(data.trust.founder_alt)}"></figure><div><p class="ap-eyebrow">WHY THIS MATTERS</p><h2>${esc(data.trust.community_heading)}</h2>${data.trust.community_paragraphs.map(p).join('')}</div></div>
<div class="ap-plain"><div><h3>Homeward is…</h3><ul class="ap-list">${data.trust.is.map((it)=>`<li>${esc(it)}</li>`).join('')}</ul></div><div><h3>Homeward is not…</h3><ul class="ap-list ap-not">${data.trust.is_not.map((it)=>`<li>${esc(it)}</li>`).join('')}</ul></div></div><p class="ap-disclaimer">${esc(data.trust.disclaimer)}</p></div></section>
